...
- 知道了这些，如果我们也想写一个简单的 `webpack `大概就有了思路：我们用我们的方式去实现以上的功能就👌了，这里简单说一下：我们可以用到一个模板（可以copy一些webpack打包出来的结果文件）用到 `ejs`编译替换模板内容，用到正则或者 `ascon` 等工具，修改 `ast`。这只是我面前能想到的思路，当然要写出webpack 考虑的那样周全，还不是那么简单的，但是至少我们知道了原理，知道了方法，这是最重要的
